...
const solutions = [
  {
    tab: 'Axxe Frame',
    heading: 'Patented display framing system designed for seamless installations.',
    description: 'Axxe Frame™ is a patented framing system designed for the future of digital displays. Engineered for seamless integration, premium aesthetics, and effortless installation. Transform any environment into a modern visual experience.',
    image: '/frame.jpeg',
    stats: [
      { value: '70%', label: 'Faster installation' },
      { value: '5x', label: 'Longer lifespan' },
    ],
  },
  {
    tab: 'Axxe Screen',
        ...
        <div className={`grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 mb-12 transition-all duration-800 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <div className="flex items-end">
            <p className="text-white/60 font-light leading-relaxed">
              Axxe seamlessly combines cutting-edge hardware, powerful software, and advanced intelligence into one truly unified ecosystem — built to scale with your business.
            </p>
          </div>
        </div>
            ...
            <div className="grid grid-cols-3 gap-3 mb-8">
              {solutions.map((s, i) => (
                <button
                  key={s.tab}
                  onClick={() => setActive(i)}
                  className={`py-3 text-xs font-medium tracking-[0.15em] uppercase border transition-all duration-300 rounded-[10px] ${
                    active === i
                      ? 'bg-gold text-midnight border-gold'
                      : 'bg-transparent text-white/70 border-gold/40 hover:border-gold'
                  }`}
                >
                  {s.tab}
                </button>
