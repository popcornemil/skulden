    ...
    <section className="bg-[#fcfbf9]" ref={ref}>
      <div className="max-w-[1200px] mx-auto w-full grid grid-cols-1 md:grid-cols-2">
        <div className="relative p-6 md:p-[50px]">
          ...
          <p className={`font-serif text-sm font-medium text-midnight mb-4 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>Request a Quote</p>
          <h2 className={`font-serif text-[32px] md:text-[48px] font-light text-midnight leading-tight mb-4 transition-all duration-1000 delay-200 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>Ready to Upgrade<br />Your Displays?</h2>
          <p className={`font-serif text-[16px] text-midnight/60 transition-all duration-1000 delay-[400ms] ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>From single screens to large-scale installations — <span className="font-bold">Axxe Screen™</span> delivers professional-grade performance for any environment. Get in touch and we'll find the right display for your project.</p>
        </div>
